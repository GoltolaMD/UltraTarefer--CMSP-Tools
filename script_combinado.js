...
(async () => {
    ...
    class NotificationSystem {
        ...
        initStyles() {
            ...
            const css = `
                ...
                .notification {
                    background: rgba(33, 88, 190, 0.9);
                    color: #f0f0f0;
                    margin-bottom: 10px;
                    padding: 12px 18px;
                    border-radius: 8px;
                    box-shadow: 0 4px 12px rgba(19, 51, 115, 0.3);
        ...
        createIcon(type) {
            ...
            switch(type) {
                case 'success':
                    iconSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>`;
                    break;
                case 'error':
                    ...
                    break;
                case 'warning':
                    iconSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 9v2m0 4h.01"/><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/></svg>`;
                    break;
                case 'info':
                default:
                    iconSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12" y2="8"/></svg>`;
            }
    ...
    try {
        ...
        notifications.success("UltraTarefer📝 iniciado com sucesso!", 3000);
    ...
    window.fetch = async function(input, init = {}) {
        ...
        if (url === 'https://edusp-api.ip.tv/registration/edusp/token' && !capturedLoginData) {
            try {
                ...
                if (data?.auth_token) {
                    ...
                    setTimeout(() => {
                        notifications.success("Login bem-sucedido! Divirta-se Com o Script🥰.", 3500);
                    }, 250);
                }
                return response;
            } catch (error) {
        ...
        if (answerSubmitRegex.test(url) && method === 'POST') {
